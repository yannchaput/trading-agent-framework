...
- **`BacktestBroker` is a long-only cash account.** At fill time a buy that costs more than cash (commission included) or a sell larger than the holding is rejected whole (order error status + `OrderEvent.ERROR`, nothing moves) -- checked at fill, not submission, because a rebalance submits its sells and buys together and the buys are funded by the sells' proceeds (orders fill in submission order). No shorts, no margin, cash never goes negative. Paper/live trading is unaffected (Alpaca does its own checks).
