"""
Cross-Sectional Momentum Strategy (V3.3) — Momentum Breadth Overlay
-------------------------------------------------------------------
Replaces the market regime overlay (SPY SMA-200) with a momentum
breadth overlay that asks: "Is my alpha factor healthy?"

Instead of checking whether SPY is bullish, this measures what
fraction of the eligible universe has positive momentum. The
intuition: cross-sectional momentum works best when there's
dispersion — when many stocks have strong positive momentum.
When few stocks show positive momentum, the factor is weak and
exposure should be reduced regardless of what SPY is doing.
...
Implements:
  ...
  7. Momentum breadth overlay — third exposure leg
  8. Combined via min(risk, vol, breadth) — most conservative wins
  9. Hysteresis: sell below rank 35, buy top 20

Breadth bands:
  Breadth >= 40% → 100% exposure
  Breadth 25–40% → 75% exposure
  Breadth 15–25% → 50% exposure
  Breadth < 15%   → 25% exposure
"""
...
from ..utils import (
    ...
    apply_filters,
    compute_atr_from_df,
    compute_momentum_breadth_exposure,
...
class CrossMomentumStrategyV3(WrappingStrategy):
    """Cross-sectional momentum strategy V3 — momentum breadth overlay.

    This strategy replaces the SPY-based market regime filter with a
    momentum breadth overlay. On each rebalance day, it measures what
    fraction of the eligible universe has positive momentum (score > 0).
    When momentum breadth is high, the alpha factor is "healthy" and
    the strategy runs at full exposure. When breadth declines, exposure
    is scaled down — protecting capital during periods when cross-sectional
    momentum doesn't differentiate well between stocks.

    The momentum breadth exposure is combined with the portfolio risk
    overlay and volatility targeting via min() — the most conservative
    leg determines the final exposure multiplier.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # V3.3: Momentum breadth overlay parameters
        self.params["momentum_breadth"] = {
            "enabled": True,
        ...
        # V3.3: Store latest momentum breadth metrics (computed on rebalance days)
        self._momentum_breadth: float | None = None
        self._breadth_exposure: float = 1.0
        self._breadth_positive_count: int = 0
        self._breadth_total_count: int = 0

        self.log_message(
            f"CrossMomentumStrategyV33 initialized with {len(self.__universe)} symbols",
            color="green",
        )
    ...
    def compute_target_portfolio(self) -> tuple[list[dict], dict[str, int]]:
        ...
        V3.3: Also computes momentum breadth from the scored universe after
        filtering, so the breadth overlay can use it in on_trading_iteration.
        ...
        # ── V3.3: Compute momentum breadth from scored universe ──────────
        breadth_config = self.params.get("momentum_breadth", {})
        if breadth_config.get("enabled", False):
            bands = breadth_config.get("bands", CONFIG["MOMENTUM_BREADTH_BANDS"])
            breadth, b_exposure, pos_count, total_count = compute_momentum_breadth_exposure(scored, bands)
            self._momentum_breadth = breadth
            self._breadth_exposure = b_exposure
            self._breadth_positive_count = pos_count
            self._breadth_total_count = total_count
            self.log_message(
                f"Momentum breadth: {breadth:.1%} ({pos_count}/{total_count} stocks with positive momentum) → {b_exposure:.0%} exposure",
                color="cyan" if b_exposure >= 1.0 else "yellow",
            )
    ...
    def on_trading_iteration(self):
        ...
        # today's target stocks' historical closes.
        risk_exposure = 1.0
        risk_metrics = {}
        ...
        # Step 5: V3.3 Momentum Breadth Overlay — scale exposure based on
        # the fraction of eligible stocks with positive momentum.
        # This is the key innovation: "Is my alpha factor healthy?"
        breadth_config = self.params.get("momentum_breadth", {})
        breadth_exposure = 1.0
        if breadth_config.get("enabled", False):
            breadth_exposure = self._breadth_exposure
            if self._momentum_breadth is not None:
                self.log_message(
                    f"Momentum breadth overlay: breadth={self._momentum_breadth:.1%} ({self._breadth_positive_count}/{self._breadth_total_count} positive), exposure={breadth_exposure:.0%}",
                    color="cyan" if breadth_exposure >= 1.0 else "yellow",
                )
            else:
                breadth_exposure = 1.0  # Default to full exposure if no breadth data
                self.log_message(
                    "Momentum breadth: no data — defaulting to 100% exposure",
                    color="yellow",
                )

        # Step 6: Combine exposures via min() — most conservative leg wins.
        # V3.3 adds breadth_exposure as a third leg in the min() chain.
        final_exposure = min(risk_exposure, vol_exposure, breadth_exposure)
        if final_exposure < 1.0:
            self.log_message(
                f"Combined exposure: {final_exposure:.0%} (risk={risk_exposure:.0%}, vol={vol_exposure:.0%}, breadth={breadth_exposure:.0%})",
                color="yellow",
            )
