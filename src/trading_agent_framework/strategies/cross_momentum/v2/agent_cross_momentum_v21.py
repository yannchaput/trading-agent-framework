"""
Cross-Sectional Momentum Strategy (V2.1)
----------------------------------
Pure rules-based weekly cross-sectional momentum strategy with a market regime overlay.
The regime filter scales portfolio exposure based on SPY's position relative to
its 200-day SMA (bull/neutral/bear). No LLM, no AI agents in v2.

Implements the methodology from docs/Cross-sectional momentum.md:
  1. Load pre-computed universe (monthly batch)
  2. Daily: compute indicators (momentum returns, volatility, ATR, dollar volume)
  3. Weekly (Friday): filter, score, rank, select top N, weight by inverse vol
  4. Hysteresis: sell below rank 35, buy top 20
  5. **Regime filter scales exposure: full in bull, reduced in neutral, cash in bear.**
  6. Target weights are scaled by the regime multiplier before rebalance.
"""
...
class CrossMomentumStrategyV21(WrappingStrategy):
    """Cross-sectional momentum strategy — weekly rebalance with market regime overlay.

    The regime filter (SPY close vs 200-day SMA) computes an exposure multiplier
    that scales target weights before rebalance. No AI agent — pure rules-based.
