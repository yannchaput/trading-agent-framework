"""
Cross-Sectional Momentum Strategy (V2.3b)
----------------------------------
V2.3a + equity-curve-based volatility targeting.

Key change from V2.3a:
  - The V2.3a risk overlay computed volatility from today's target stocks'
    historical closes — a hypothetical "risk if I had held this portfolio."
  - V2.3b tracks the strategy's ACTUAL daily equity curve, computes realized
    portfolio volatility from it, and targets a configurable vol level
    (default 20% annualized).
...
Implements:
  ...
  6. V2.3b volatility targeting (equity-curve-based) — second leg
...
class CrossMomentumStrategyV23b(WrappingStrategy):
    """Cross-sectional momentum strategy V2.3b — equity-curve volatility targeting.

    Tracks actual daily portfolio equity to compute realized volatility,
    then scales exposure to target a configurable vol level (default 20%).
    Combined with the V2.3a risk overlay via min() — the most conservative
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # V2.3b: enable and configure volatility targeting
        ...
        # V2.3b: track actual portfolio equity for realized-vol computation
        self._equity_history: list[float] = load_equity_history()

        self.log_message(
            f"CrossMomentumStrategyV23b initialized with {len(self.__universe)} symbols",
            color="green",
        )
    ...
    # ── Realized volatility from equity curve (V2.3b) ────────────────────────

    def _compute_realized_volatility(self) -> float | None:
        """Compute annualized 20d realized vol from actual portfolio equity.

        Uses the strategy's own equity curve (self._equity_history) rather than
        reconstructing hypothetical returns from today's target stocks.
        This measures the volatility the portfolio ACTUALLY experienced.

        Returns:
            Annualized volatility as a float, or None if insufficient history.
        """
        vt_config = self.params.get("volatility_targeting", {})
        lookback = vt_config.get("lookback_days", 20)

        if len(self._equity_history) < lookback + 1:
            return None
    ...
    def on_trading_iteration(self):
        ...
        # V2.3b: record daily equity for realized-vol tracking
        ...
        # Step 4: V2.3b Volatility Targeting — compute realized vol from actual
        # equity curve and derive a second exposure multiplier.
        vt_config = self.params.get("volatility_targeting", {})
        vol_exposure = 1.0
        realized_vol = None
        if vt_config.get("enabled", False):
            ...
            if realized_vol is not None:
                ...
                self.log_message(
                    f"Vol targeting: realized_vol={realized_vol:.1%}, target={vt_config['target_volatility']:.0%}, vol_exposure={vol_exposure:.0%}",
                    color="cyan",
                )
            else:
                self.log_message(
                    f"Vol targeting: insufficient equity history ({len(self._equity_history)} days, need {vt_config.get('lookback_days', 20) + 1}) — defaulting to 100% exposure",
                    color="yellow",
                )
