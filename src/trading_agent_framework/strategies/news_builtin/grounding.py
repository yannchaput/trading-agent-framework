...
first. The prompt's own workflow already puts "scan broad-market news" before deciding or trading (step
2), but wording alone did not reliably get it followed. This wraps the assembled tool list to enforce
that ordering in code: a call to either gated tool is refused, without running, until `search_news` has
returned a non-error result earlier in the same agent run (by `current_run_id()`, mirroring
`agents/tools/trading.py`'s `_OrdersThisRun`). Outside an agent run nothing is gated, same convention as
every other per-run tool tracker in this codebase.

An earlier version of this gate also required that call to resolve `include_content=True` and return an
article with actual body content, to stop the model judging a call from headlines alone. That backfired:
...
side, no matter how the search window is narrowed -- so the requirement was sometimes impossible to
satisfy. The model kept retrying different articles as its own refusal message instructed, all refused,
sometimes 100+ tool calls in a single run (`agent_news_binary.py`'s `_decision_was_recorded` warning and
`AgentManager`'s tool-call-repair middleware both exist to catch what running out of turn/token budget
that way looks like -- neither can produce a decision that was never attempted, only recover one that
was). Deciding from a headline alone is an accepted tradeoff, not a gap to close: the gate only requires
that `search_news` ran at all.
"""
