...
def _build_system_prompt(*, symbols: Sequence[str], defensive_symbol: str, news_symbols: str) -> str:
    ...
    return (
        ...
        f"Going from {defensive_symbol} back to {risky} needs one clear bullish signal. Staying in the current "
        "holding needs no signal at all -- the signal counts above apply only to the specific transition you are "
        "making, never to remaining where you already are. If current_regime is 'mixed', finish the rotation your "
        "last recorded decision started.\n"
