...
def _parse_tearsheet_csv(path: str) -> dict[str, dict[str, float]] | None:
    """Parse scalar metrics from a *_tearsheet.csv file.

    The CSV tearsheet is generated by quantstats via ``qs.reports.html()``
    with ``match_dates=False``, which preserves the full return series (day-0
    anchor, no truncation to first non-zero return). The JSON tearsheet
    (``*_tearsheet_metrics.json``) is generated by ``qs.reports.metrics_json()``
    which *defaults* to ``match_dates=True``, causing ``_match_dates`` to trim
    both strategy and benchmark to the later of their first non-zero return
    dates. When a strategy sits in cash for the first year, this can inflate
    annualized metrics by 10+ percentage points (e.g. CAGR 21.6% → 30.8%).

    Returns a dict shaped like ``scalar_metrics`` (metric_name → {column: value})
    so it can be fed directly to ``MetricSet.from_scalars()``. The CSV benchmark
    column (typically the benchmark ticker like "SPY") is renamed to "Benchmark" to match the JSON naming
    convention expected by ``from_scalars()``.
    """
    try:
        df = pd.read_csv(path)
    except Exception:
    ...
    if df.empty or df.columns[0] != "Metric":
        return None

    # Identify value columns and map the benchmark column → "Benchmark" to match from_scalars() convention.
    rename: dict[str, str] = {}
    ...
    if rename:
        ...
        # Recompute value_columns after rename
        value_columns = [rename.get(c, c) for c in value_columns]

    scalar_metrics: dict[str, dict[str, float]] = {}
    for _, row in df.iterrows():
        metric_name = str(row["Metric"]).strip() if pd.notna(row["Metric"]) else ""
        if not metric_name:
        ...
        for col in value_columns:
            raw = row.get(col)
            val = _parse_tearsheet_cell(raw)
            if val is not None:
                entry[col] = val

        if entry:
            scalar_metrics[metric_name] = entry

    return scalar_metrics if scalar_metrics else None


def _parse_tearsheet_cell(raw: object) -> float | None:
    """Parse a single tearsheet CSV cell into a float.

    Handles percentage strings ("52.28%" → 0.5228), comma-separated
    numbers ("1,304" → 1304), and dash/null placeholders ("-" → None).
    """
    if raw is None:
        return None
    s = str(raw).strip()
...
def load_metrics(ref: RunRef) -> MetricSet | None:
    """Load performance metrics, preferring the CSV tearsheet for accuracy.

    The CSV tearsheet (generated with match_dates=False) has the correct
    full-period metrics. The JSON tearsheet (generated with match_dates=True
    by default in quantstats) can inflate annualized metrics when the
    strategy has a long initial cash-only period.

    Scalar metrics come from the CSV (falling back to JSON if CSV is missing
    or unparseable). Summary tables (yearly returns, drawdowns) come from
    the JSON since the CSV doesn't include them.
    """
    # --- JSON (for summary_tables and as scalar fallback) ---
    json_path = _find_file(ref.path, "*_tearsheet_metrics.json")
    ...
    # --- CSV (preferred source for scalar metrics) ---
    csv_path = _find_file(ref.path, "*_tearsheet.csv")
    scalar_metrics: dict[str, dict[str, float]] | None = None
    if csv_path is not None:
        scalar_metrics = _parse_tearsheet_csv(csv_path)

    # Fall back to JSON scalar_metrics if CSV parsing failed
    if scalar_metrics is None and json_data is not None:
        scalar_metrics = json_data.get("scalar_metrics", {})

    if scalar_metrics is None:
        return None

    summary_tables = json_data.get("summary_tables") if json_data else None
    return MetricSet.from_scalars(scalar_metrics, summary_tables=summary_tables)
